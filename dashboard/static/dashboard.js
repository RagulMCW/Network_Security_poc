...
async function downloadLLMLogsAsHTML() {
    try {
        ...
        if (llmResponses.length === 0) {
            showToast('No LLM responses to download', 'error');
            return;
        }
...
<head>
    ...
    <title>LLM AI Responses - Honeypot Report</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #1a1a2e; color: #eee; padding: 2rem; }
        h1 { text-align: center; color: #3498db; margin-bottom: 0.5rem; }
        .subtitle { text-align: center; color: #888; margin-bottom: 2rem; }
        .summary { background: #16213e; padding: 1rem; border-radius: 8px; margin-bottom: 2rem; display: flex; justify-content: space-around; }
        .summary-item { text-align: center; }
        .summary-item h3 { color: #3498db; font-size: 2rem; }
        .summary-item p { color: #888; }
        .log-entry { background: #16213e; border-left: 4px solid #3498db; padding: 1rem; margin-bottom: 1rem; border-radius: 0 8px 8px 0; }
        .header { display: flex; justify-content: space-between; margin-bottom: 0.75rem; flex-wrap: wrap; gap: 0.5rem; }
        .badge { background: #3498db; color: white; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.8rem; }
        .time { color: #888; font-size: 0.85rem; }
        .user-info { color: #aaa; margin-bottom: 0.5rem; }
        .command { background: #0d1117; padding: 0.5rem; border-radius: 4px; margin-bottom: 0.5rem; }
        .command strong { color: #f39c12; }
        .response { background: #0d1117; padding: 0.75rem; border-radius: 4px; font-family: 'Courier New', monospace; color: #2ecc71; white-space: pre-wrap; word-wrap: break-word; }
        .footer { text-align: center; color: #666; margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #333; }
    </style>
</head>
<body>
    <h1>🧠 LLM AI Responses Report</h1>
    <p class="subtitle">Honeypot Interaction Log - Generated ${new Date().toLocaleString()}</p>
    
    <div class="summary">
        <div class="summary-item">
            <h3>${llmResponses.length}</h3>
            <p>Total Interactions</p>
        </div>
        <div class="summary-item">
            <h3>${[...new Set(llmResponses.map(e => e.source_ip))].length}</h3>
            <p>Unique Attackers</p>
        </div>
        <div class="summary-item">
            <h3>${[...new Set(llmResponses.map(e => e.command))].length}</h3>
            <p>Unique Commands</p>
        </div>
    </div>
    
    ${llmResponses.map(entry => {
        const time = new Date(entry.time).toLocaleString();
        const command = entry.command || 'N/A';
        const aiResponse = entry.ai_response || 'No response';
    ...
    <div class="log-entry">
        <div class="header">
            ...
            <span class="badge" style="background: #9b59b6;">${protocol}</span>
            <span class="time">📅 ${time}</span>
        </div>
        <div class="user-info">👤 ${user}@${sourceIp}</div>
        <div class="command"><strong>$ ${command.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</strong></div>
        <div class="response">${aiResponse.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
    </div>`;
    }).join('\n')}
    
    <div class="footer">
        <p>Generated by Network Security Dashboard | Honeypot: Beelzebub</p>
    </div>
</body>
</html>`;
        ...
        a.href = url;
        a.download = `llm_responses_${new Date().toISOString().slice(0,10)}.html`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        
        showToast(`Downloaded ${llmResponses.length} LLM responses as HTML`, 'success');
        
    } catch (error) {
        console.error('Error downloading LLM logs:', error);
        showToast('Error downloading LLM logs', 'error');
    }
}
